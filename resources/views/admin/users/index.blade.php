...
@section('page_header')
    ...
    <a href="{{ route('admin.users.create') }}" class="vx-btn vx-btn-primary">
                ...
                <tbody>
                    @forelse ($users as $u)
                        <tr>
                            ...
                            <td>
                                @if ($password = $staffPasswords[$u->email] ?? null)
                                    <code class="rounded bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-800">{{ $password }}</code>
                                @else
                            ...
                            <td class="vx-text-end">
                                <div class="vx-table-actions justify-end">
                                    <a href="{{ route('admin.users.edit', $u) }}" class="vx-btn-icon" aria-label="Edit pengguna" title="Edit">
                                    ...
                                    <form action="{{ route('admin.users.destroy', $u) }}" method="POST" class="inline" onsubmit="return confirm('Hapus pengguna ini?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="vx-btn-icon is-danger" aria-label="Hapus pengguna" title="Hapus">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center text-sm text-slate-500 py-10">Belum ada pengguna.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
